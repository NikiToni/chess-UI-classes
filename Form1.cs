...
namespace chess_drag_test
{
 ...
 //           pictureBox1.Invalidate();
        ...
        private void pictureBox1_Paint_1(object sender, PaintEventArgs e)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                for (int j = 0; j < 8; j++)
                {
                    if (!darkSquare)
                    {
                        //e.Graphics.FillRectangle(rectangleFillDark, i * ChessBoard.CellWidth, j * ChessBoard.CellHeight, ChessBoard.CellWidth, ChessBoard.CellHeight);
                    }
                    else
                    {
        ...
        {
            set { darkSquare = value == Colors.white ? true : false; }
        }
